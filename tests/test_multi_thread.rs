...
use chrono::Local;
use flexi_logger::{init,LogConfig,LogRecord};
use glob::glob;
...
#[test]
fn multi_threaded() {
    println!("create a huge number of log lines with a significant number of threads, verify the log");
    ...
    init(
        LogConfig {
            ...
            rotate_over_size    : Some(ROTATE_OVER_SIZE),
            .. LogConfig::new()
        },
...
fn start_worker_threads(no_of_workers: usize) -> Vec<JoinHandle<u8>> {
    ...
    for thread_number in 0..no_of_workers {
        ...
        worker_handles.push(
            ...
            .spawn(move || {do_work(thread_number);0 as u8})
            .unwrap()
        );
    }
...
fn verify_logs(directory: String) {
    ...
    for globresult in globresults {
        ...
        let f = File::open(&pathbuf).unwrap_or_else(|e|{panic!("Cannot open file {:?} due to {}", pathbuf,e)});
        no_of_log_files += 1;
    ...
    println!("Wrote {} log lines from {} threads into {} files",
            NO_OF_THREADS * NO_OF_LOGLINES_PER_THREAD + 1, NO_OF_THREADS, no_of_log_files);
}
